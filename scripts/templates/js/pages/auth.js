/**
 * =============================================================================
 * PAGE D'AUTHENTIFICATION - MARKETFLOW LOGIN / REGISTER
 * =============================================================================
  ...
  return `
        <div class="auth-page">
            <div class="auth-container">
                <!-- Logo mis à jour pour MarketFlow -->
                <div class="auth-logo">
                ...
                <p class="auth-subtitle">
                    ${
                      isLogin
                        ...
                        : "Rejoignez notre communauté d'acheteurs et de vendeurs"
                    }
                </p>
                ...
                <p class="auth-switch mt-lg">
                    ${
                      isLogin
                        ? 'Pas encore de compte ? <a href="#" class="auth-link" onclick="event.preventDefault(); navigateTo(\'register\')">S\'inscrire</a>'
                        : 'Déjà un compte ? <a href="#" class="auth-link" onclick="event.preventDefault(); navigateTo(\'login\')">Se connecter</a>'
                    }
                </p>
  ...
  return `
        <form class="auth-form" onsubmit="handleLogin(event)">
            <div class="form-group">
                <label class="form-label" for="login-email"><i class="bi bi-envelope"></i> Email</label>
                <input type="email" id="login-email" class="form-input" 
                       placeholder="votre@email.com" required>
            </div>
            
            <div class="form-group">
                <label class="form-label" for="login-password"><i class="bi bi-lock"></i> Mot de passe</label>
        ...
        <div style="display: flex; flex-direction: column; gap: var(--spacing-sm);">
            <button class="btn btn-secondary w-full" onclick="quickLogin('client')">
                <i class="bi bi-person"></i> Connexion Client (démo)
            </button>
            <button class="btn btn-secondary w-full" onclick="quickLogin('seller')">
                <i class="bi bi-shop-window"></i> Connexion Vendeur (démo)
            </button>
            <button class="btn btn-secondary w-full" onclick="quickLogin('admin')">
                <i class="bi bi-gear"></i> Connexion Admin (démo)
            </button>
        </div>
    `
}
...
function renderRegisterForm() {
  return `
        <form class="auth-form" onsubmit="handleRegister(event)">
            ...
            <div class="form-group">
                <label class="form-label" for="register-email"><i class="bi bi-envelope"></i> Email</label>
                <input type="email" id="register-email" class="form-input" 
                       placeholder="votre@email.com" required>
            </div>
            
            <div class="form-group">
                <label class="form-label" for="register-password"><i class="bi bi-lock"></i> Mot de passe</label>
                <input type="password" id="register-password" class="form-input" 
                       placeholder="Minimum 6 caractères" required minlength="6">
            </div>
            
            <div class="form-group">
                <label class="form-label" for="register-role"><i class="bi bi-person-badge"></i> Type de compte</label>
...
async function quickLogin(role) {
  const credentials = {
    client: { email: "client@marketflow.com", password: "client123" },
    seller: { email: "vendeur@marketflow.com", password: "vendeur123" },
    admin: { email: "admin@marketflow.com", password: "admin123" },
  }
